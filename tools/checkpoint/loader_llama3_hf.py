...
def set_postprocess_state(args, model, hf_model):
    ...
    hf_model.model.norm.weight.to(model.decoder.final_layernorm.weight.dtype)
)
...
def set_attn_state(args, layer, hf_layer):
    ...
    G = A if not args.group_query_attention else args.num_query_groups
    
    d = H // A; group = A // G
    attn = layer.self_attention
    q = hf_layer.self_attn.q_proj.weight          # (H,H)
    k = hf_layer.self_attn.k_proj.weight          # (G*d,H)
    v = hf_layer.self_attn.v_proj.weight          # (G*d,H)
    o = hf_layer.self_attn.o_proj.weight          # (H,H)

    W = attn.linear_qkv.weight
    shape = tuple(W.shape)

    if shape == (3*H, H):                     # 旧: [Q;K;V] x In
        # KVをA展開してHに揃える
        k_full = k.view(G, d, H).repeat_interleave(A//G, dim=0).reshape(H, H)
        v_full = v.view(G, d, H).repeat_interleave(A//G, dim=0).reshape(H, H)
        fused = torch.cat([q, k_full, v_full], dim=0)           # (3H, H)
        W.data.copy_(fused.to(W.dtype))

    elif shape == (H, 3*H):                   # 旧: Out x [Q|K|V]
        k_full = k.view(G, d, H).repeat_interleave(A//G, dim=0).reshape(H, H)
        v_full = v.view(G, d, H).repeat_interleave(A//G, dim=0).reshape(H, H)
        fused = torch.cat([q, k_full, v_full], dim=0).t()       # (H, 3H)
        W.data.copy_(fused.to(W.dtype))

    elif shape == (H + 2*G*d, H):             # **GQA前提**: [Q;(K_G);(V_G)] x In
        fused = torch.cat([q, k, v], dim=0)                     # (H+2Gd, H)
        W.data.copy_(fused.to(W.dtype))

    elif shape == (H, H + 2*G*d):             # Out x [Q|K_G|V_G]
        fused = torch.cat([q, k, v], dim=0).t()                 # (H, H+2Gd)
        W.data.copy_(fused.to(W.dtype))

    else:
        raise RuntimeError(f"unexpected linear_qkv shape {shape}")
    
    # 出力Proj
    w = attn.linear_proj.weight
    attn.linear_proj.weight.data.copy_(o.to(w.dtype if w.shape==o.shape else o.t().dtype))
        

def set_mlp_state(args, layer, hf_layer):
    mlp = layer.mlp
    up = hf_layer.mlp.up_proj.weight        # (I, H)
    gate = hf_layer.mlp.gate_proj.weight    # (I, H)
    down = hf_layer.mlp.down_proj.weight    # (H, I)

    # Megatron-Core: fc1 は [gate; up] を結合した列並列(2I, H) または転置(H, 2I)
    w1 = torch.cat([gate, up], dim=0)       # (2I, H)

    if hasattr(mlp, "linear_fc1") and hasattr(mlp, "linear_fc2"):
        W1 = mlp.linear_fc1.weight
        if W1.shape == w1.shape:            # (2I, H)
            W1.data.copy_(w1.to(W1.dtype))
        elif W1.shape == (w1.shape[1], w1.shape[0]):  # (H, 2I)
            W1.data.copy_(w1.t().to(W1.dtype))
        else:
            raise RuntimeError(f"unexpected fc1 shape {tuple(W1.shape)}")

        W2 = mlp.linear_fc2.weight          # down proj
        if W2.shape == down.shape:          # (H, I)
            W2.data.copy_(down.to(W2.dtype))
        elif W2.shape == (down.shape[1], down.shape[0]):  # (I, H)
            W2.data.copy_(down.t().to(W2.dtype))
        else:
            raise RuntimeError(f"unexpected fc2 shape {tuple(W2.shape)}")

    else:
        # 旧Megatron互換（あれば）
        if hasattr(mlp, "dense_h_to_4h") and hasattr(mlp, "dense_4h_to_h"):
            mlp.dense_h_to_4h.weight.data.copy_(w1.to(mlp.dense_h_to_4h.weight.dtype))
            mlp.dense_4h_to_h.weight.data.copy_(down.to(mlp.dense_4h_to_h.weight.dtype))
        else:
            raise AttributeError("MLP neither Core(linear_fc1/2) nor legacy(dense_*).")


def safe_copy_norm(dst_norm, src_norm):
    if hasattr(dst_norm, "weight") and hasattr(src_norm, "weight"):
        dst_norm.weight.data.copy_(src_norm.weight.to(dst_norm.weight.dtype))
...
def set_layer_state(args, model, hf_model, layer_idx):
    ...
    safe_copy_norm(layer.input_layernorm, hf_layer.input_layernorm)
    safe_copy_norm(layer.pre_mlp_layernorm, hf_layer.post_attention_layernorm)
...
def _load_checkpoint(queue, args):
    ...
    for layer_num in range(margs.num_layers):
        ...
        layer = model.decoder.layers[layer_num]
        attn = layer.self_attention
        mod = getattr(attn, "query_key_value", None) or getattr(attn, "linear_qkv", None)
        assert mod is not None, "no fused QKV module on SelfAttention"
        qkv_weight.append(mod.weight.data)
        proj = getattr(attn, "dense", None) or getattr(attn, "linear_proj", None)
        assert proj is not None, "no fused dense module on SelfAttention"
        dense_weight.append(proj.weight.data)
        mlp = layer.mlp
        fc1 = getattr(mlp, "dense_h_to_4h", None) or getattr(mlp, "linear_fc1", None)
        fc2 = getattr(mlp, "dense_4h_to_h", None) or getattr(mlp, "linear_fc2", None)
        assert fc1 is not None and fc2 is not None, "no fc1/fc2 MLP weights found"
        mlp_l0_weight.append(fc1.weight.data)
        mlp_l1_weight.append(fc2.weight.data)
        if md.linear_bias:
            qkv_bias.append(mod.bias.data)
            mlp_l0_bias.append(fc1.bias.data)

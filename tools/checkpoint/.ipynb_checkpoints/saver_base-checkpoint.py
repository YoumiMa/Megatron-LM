...
class MegatronCheckpointSaverBase:
    ...
    def parse_megatron_args(self):
        ...
        if hasattr(self.md, 'rope_scaling') and self.md.rope_scaling is not None:
            margs.rope_scaling = self.md.rope_scaling
        elif hasattr(self.md, 'checkpoint_args') and hasattr(self.md.checkpoint_args, 'rope_scaling'):
            margs.rope_scaling = self.md.checkpoint_args.rope_scaling
        # ============ 追加ここまで ============
    ...
    def initialize_megatron_env(self):
        ...
        # ============ 追加: RoPE設定を復元 ============
        ckpt_args = self.md
        if hasattr(ckpt_args, 'rope_theta'):
            self.margs.rope_theta = ckpt_args.rope_theta
            print(f"Setting rope_theta to {self.margs.rope_theta}")
        if hasattr(ckpt_args, 'rope_scaling'):
            self.margs.rope_scaling = ckpt_args.rope_scaling
            print(f"Setting rope_scaling to {self.margs.rope_scaling}")
        # ============ 追加ここまで ============
    ...
    def build_sys_argv(self): 
        ...
        # # ============ 追加: RoPE設定 ============
        # # rope_theta
        # rope_theta = getattr(self.md, 'rope_theta', None)
        # if rope_theta is None and hasattr(self.md, 'checkpoint_args'):
        #     rope_theta = getattr(self.md.checkpoint_args, 'rope_theta', None)
        # if rope_theta is not None:
        #     my_argv.extend(['--rotary-base', str(rope_theta)])
    
        # # rope_scaling
        # rope_scaling = getattr(self.md, 'rope_scaling', None)
        # if rope_scaling is None and hasattr(self.md, 'checkpoint_args'):
        #     rope_scaling = getattr(self.md.checkpoint_args, 'rope_scaling', None)
        # if rope_scaling is not None and isinstance(rope_scaling, dict):
        #     # Megatronのコマンドライン引数形式に変換
        #     rope_type = rope_scaling.get('rope_type') or rope_scaling.get('type')
        #     if rope_type:
        #         my_argv.extend(['--rotary-scaling-type', str(rope_type)])
        #     factor = rope_scaling.get('factor')
        #     if factor is not None:
        #         my_argv.extend(['--rotary-scaling-factor', str(factor)])
        #     # 追加のスケーリングパラメータ（Megatronがサポートしている場合）
        #     # low_freq_factor, high_freq_factor, original_max_position_embeddings など
        # # ============ 追加ここまで ============
